 ...
 * @author max
 */
@WebServlet(name = "GetCurrentEvents", urlPatterns =
{
    "/getcurrentevents"
})
public class GetCurrentEvents extends HttpServlet
{
    private static final Logger log = LoggerFactory.getLogger(GetCurrentEvents.class);
    ...
    {
        log.trace("doGet()");
        List currentEvents = DatabaseAccess.getCurrentEvents();
        StandardOutputObject output = new StandardOutputObject();

        if (currentEvents != null)
        {
            output.setSuccess(true);
            output.setData(currentEvents);
